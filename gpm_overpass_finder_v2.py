...
SEGMENT_HOURS = 24                     # split temporal search into segments; set None to disable
RADIUS_KM = 245                    # "scanned" threshold around storm center
BBOX_BUFFER_DEG = 2.5                 # bbox buffer added around track bbox (degrees)
...
def run_for_year(season_year: int):
    ...
    for sid, sdf in storms:
        ...
        # Split long temporal windows into segments to avoid CMR truncation.
        if SEGMENT_HOURS is None:
            segments = [(tmin, tmax)]
        else:
            segments = []
            step = pd.Timedelta(hours=SEGMENT_HOURS)
            if tmax <= tmin:
                segments = [(tmin, tmax)]
            else:
                seg_start = tmin
                while seg_start < tmax:
                    seg_end = min(seg_start + step, tmax)
        ...
        # 3) CMR search granules (coarse filter), segment then de-dup by granule filename
        all_granules = []
        for seg_start, seg_end in segments:
            seg_df = sdf[(sdf["time_utc"] >= seg_start) & (sdf["time_utc"] <= seg_end)]
            if seg_df.empty:
                print(f"Segment skipped (no track points): {seg_start} -> {seg_end}")
                continue

            # Segment bbox + buffer
            lat_min = float(seg_df["lat"].min() - BBOX_BUFFER_DEG)
            lat_max = float(seg_df["lat"].max() + BBOX_BUFFER_DEG)

            # Handle lon bbox carefully (dateline): use unwrapped then min/max then wrap.
            lon_u = _unwrap_lon_deg(seg_df["lon"].to_numpy())
            ...
            print(f"Segment window (UTC): {seg_start} -> {seg_end}")
            print(f"Segment BBox: ({lon_min:.2f}, {lat_min:.2f}, {lon_max:.2f}, {lat_max:.2f})")
            seg_granules = earthaccess.search_data(
                short_name=GPM_SHORT_NAME,
                temporal=(seg_start.isoformat(), seg_end.isoformat()),
                bounding_box=(lon_min, lat_min, lon_max, lat_max),
            )
            all_granules.extend(seg_granules)
