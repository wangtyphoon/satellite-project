...
# -*- coding: utf-8 -*-
"""
Add quadrant-mean zFactorFinal features (DR/UR/UL/DL) using ERA5 shear direction.
"""
...
def compute_quadrant_means(
    ...
    out = {}
    for q_id, q_name in ((1, "DR"), (2, "UR"), (3, "UL"), (4, "DL")):
        vals = z_grid[(quadrants == q_id) & mask]
        out[q_name] = _apply_agg(vals, agg)
    return out
...
def main() -> None:
    ...
    quad_cols = {
        "DR": f"zFactorFinal_{AGG}_r{radius_label}_DR",
        "UR": f"zFactorFinal_{AGG}_r{radius_label}_UR",
        "UL": f"zFactorFinal_{AGG}_r{radius_label}_UL",
        "DL": f"zFactorFinal_{AGG}_r{radius_label}_DL",
    }
    for col in quad_cols.values():
    ...
    for row_idx in indices:
        row = df.loc[row_idx]
        if pd.notna(row[quad_cols["DR"]]) and pd.notna(row[quad_cols["DL"]]):
            continue
        ...
        quad_means = compute_quadrant_means(z_grid, shear_dir, RADIUS_KM, GRID_KM, AGG)

        for name, col in quad_cols.items():
            df.at[row_idx, col] = quad_means[name]
