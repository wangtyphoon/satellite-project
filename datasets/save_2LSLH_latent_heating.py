...
# -*- coding: utf-8 -*-
"""
Save 2LSLH (latent heating) as storm-centered axis-symmetric radial averages.
...
# - Dataset path uses 2LSLH latent heating candidates.
# - Single latent heating variable (no NW/DM split).
# - No vertical aggregation; keep bins when LH has a vertical dimension.
# - Debug prints for data.shape and attribute keys after reading LH.
...
DATASET_CANDIDATES = ["latentHeating"]
...
def resolve_swath(h5, preferred=None):
    ...
    candidates.extend(["Swath", "FS", "NS", "MS", "HS"])
    seen = set()
    for s in candidates:
...
def _output_path(granule_file, swath) -> str:
    out_dir = os.path.join(_script_dir(), "2LSLH")
    os.makedirs(out_dir, exist_ok=True)
    stem = Path(granule_file).stem
    tag = f"radial{int(MAX_RADIUS_KM)}km"
    return os.path.join(out_dir, f"{stem}_{swath}_2LSLH_{tag}.npy")
...
def _init_mp_context(lh, x_km, y_km, valid_xy, swath_mask, step, size, half, r_km, edges):
    global _MP_CONTEXT
    _MP_CONTEXT = {
        "lh": lh,
        "x_km": x_km,
...
def _regrid_bin_mp(b_idx: int):
    ...
    radial = _radial_profile(lh_b, ctx["r_km"], ctx["edges"])
    return b_idx, radial


def _process_row(row, row_idx):
    year = _infer_year_from_row(row)
    granule_file_raw = str(row[GRANULE_COL]).strip()
    granule_file_raw = granule_file_raw.replace("\\", "/")
    if os.path.isabs(granule_file_raw) or re.match(r"^[A-Za-z]:", granule_file_raw):
        granule_file_raw = Path(granule_file_raw).name
    granule_file = Path(granule_file_raw).name
    ...
    lh_granule_raw = re.sub(
        r"2A\.GPM\.DPR\.V9-\d+\.",
        "2A.GPM.DPR.GPM-SLH.",
        granule_file_raw,
    )
    granule_path = os.path.join(download_dir, lh_granule_raw)
    ...
    with h5py.File(granule_path, "r") as h5:
        ...
        if data_path is None:
            print(f"[{row_idx}] No dataset found under {swath} for {DATASET_CANDIDATES}.")
            keys = []
            h5.visit(keys.append)
            print(f"[{row_idx}] HDF5 keys: {keys}")
            raise ValueError(f"No dataset found under {swath} for {DATASET_CANDIDATES}.")
        ds = h5[data_path]
        data = ds[...]
        attrs = {k: ds.attrs[k] for k in ds.attrs.keys()}

    print(f"[{row_idx}] LH shape: {data.shape}")
    print(f"[{row_idx}] LH attrs: {sorted(attrs.keys())}")
    ...
    if lh.ndim == 2:
        grid_lh = _regrid_field(lh, x_km, y_km, valid_xy, swath_mask, step, GRID_SIZE, half)
        out = _radial_profile(grid_lh, r_km, edges)
    else:
        n_bins = lh.shape[2]
        n_rad = len(edges) - 1
        lh_rad = np.full((n_bins, n_rad), np.nan, dtype=np.float32)
        ...
        if USE_PARALLEL and n_bins > 1:
            ...
            if USE_MULTIPROCESS:
                with ProcessPoolExecutor(
                    ...
                    initargs=(lh, x_km, y_km, valid_xy, swath_mask, step, GRID_SIZE, half, r_km, edges),
                ) as ex:
                    futures = {ex.submit(_regrid_bin_mp, b): b for b in range(n_bins)}
                    for fut in as_completed(futures):
                        b_idx, radial = fut.result()
                        lh_rad[b_idx] = radial
                        done += 1
                ...
                with ThreadPoolExecutor(max_workers=N_WORKERS) as ex:
                    ...
                    for fut in as_completed(futures):
                        b_idx, lh_b = fut.result()
                        lh_rad[b_idx] = _radial_profile(lh_b, r_km, edges)
                        done += 1
                        if PRINT_EVERY_BIN and done % PRINT_EVERY_BIN == 0:
                            dt = time.perf_counter() - t0
                            print(f"[{row_idx}] bins {done}/{n_bins} done ({dt:.1f}s)")
